...
if x <= 0 or y <= 0 or z <= 0:
    ...
    #Заполнение матрицы
    a = [[[0.0] * x for _ in range(y)] for _ in range(z)]
    for i in range(z):
        for j in range(y):
            for k in range(x):
                a[i][j][k] = float(input(f'Введите элемент с z = {i + 1} и y = {j + 1} и x = {k + 1}: '))

    ind = int(input('Введите индекс среза матрицы: '))
    
    #Проверка корректности номера среза, затем вывод среза
    if not(0 < ind <= y):
        print('Индекс среза матрицы должен быть от 1 до y')
    else:
        for i in range(z):
                for k in range(x):
                    print(f'Элемент с z = {i + 1} и y = {ind} и x = {k + 1}: {a[i][ind-1][k]:.5g}')
